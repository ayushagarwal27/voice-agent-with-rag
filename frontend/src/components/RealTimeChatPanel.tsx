...
export default function RealTimeChatPanel({
  ...
  const handleConnect = async () => {
    ...
    if (
      ...
      console.log(
        "Client already connected/connecting, disconnecting first...",
      );
      try {
    ...
    const endpoint =
      import.meta.env.VITE_PIPECAT_ENDPOINT || "/api/v1/stream/connect";
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Client.connect() called, waiting for transport state update...",
        );
        ...
        if (
          connectError?.message?.includes("enumerateDevices") ||
          connectError?.toString().includes("enumerateDevices")
        ) {
          console.warn(
            "⚠️ Microphone access error (this is expected if not using HTTPS or microphone not available):",
            connectError,
          );
          // Try to connect anyway - the client might still work without microphone
          // The error might be non-fatal
          console.log(
            "Attempting to continue connection despite microphone error...",
          );
        } else {
          // Log the error but don't throw - let the connection attempt continue
          console.error(
            "Connection error (non-fatal, continuing):",
            connectError,
          );
        }
      }
      ...
      setTimeout(() => {
        ...
        if (
          currentState !== TransportStateEnum.READY &&
          currentState !== TransportStateEnum.CONNECTED &&
          currentState !== TransportStateEnum.CONNECTING
        ) {
          console.warn(
            "⚠️ Transport state did not transition to READY. Current state:",
            currentState,
          );
        }
      }, 5000);
            ...
            <PipecatClientMicToggle disabled={!isConnected}>
              {({ disabled, isMicEnabled, onClick }) => (
                <button
                  ...
                  title={
                    isMicEnabled ? "Microphone enabled" : "Microphone disabled"
                  }
                >
                  {isMicEnabled ? (
      ...
      <div className="border-t border-slate-700 bg-slate-800 p-4">
        <div className="flex items-center gap-3 max-w-4xl mx-auto">
          <input
            ...
            placeholder={
              isConnected ? "Send a message..." : "Connect to send messages"
            }
            disabled={!isConnected}
